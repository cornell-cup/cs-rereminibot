...
def main():
    ...
    predict_x_offset_x_input_only = predictors[
        "x_offsets_predictor_x_input_only"
    ].predict
    predict_y_offset_y_input_only = predictors[
        "y_offsets_predictor_y_input_only"
    ].predict
    ...
    while True:
        ...
        data_for_BS = {
            "DEVICE_ID": str(BASE_STATION_DEVICE_ID),
            "TIMESTAMP": time.time(),
            "DEVICE_CENTER_X": FRAME_WIDTH / 2,
            "DEVICE_CENTER_Y": FRAME_HEIGHT / 2,
            "position_data": [],
        }
        snapshot_locations = []
        for i, d in enumerate(detections):
            ...
            (detected_x, detected_y, detected_z, detected_angle) = (
                util.compute_tag_undistorted_pose(
            ...
            x_offset, y_offset, _ = get_x_y_angle_offsets(
                detected_x, detected_y, center_cell_offsets
            )
            ...
            # displaying tag id
            # cv2.putText(undst, str(d.tag_id),(int(ctr_x), int(ctr_y)), cv2.FONT_HERSHEY_SIMPLEX, .4,  (215, 113, 102),2)
            # cv2.putText(undst, str((round(x),round(y))),(int(ctr_x), int(ctr_y)+15), cv2.FONT_HERSHEY_SIMPLEX, .3,  (85, 150, 0),1)
            # cv2.putText(undst, str(round(angle)),(int(ctr_x), int(ctr_y)+30), cv2.FONT_HERSHEY_SIMPLEX, .4,  (20, 45, 125),2)

            _, h = draw_text(
                undst,
                str(d.tag_id),
                pos=(int(ctr_x), int(ctr_y)),
                text_color=(215, 113, 102),
            )
            _, h1 = draw_text(
                undst,
                str((round(x), round(y))),
                pos=(int(ctr_x), int(ctr_y) + h),
                text_color=(85, 150, 0),
            )
            draw_text(
                undst,
                str(round(angle)),
                pos=(int(ctr_x), int(ctr_y) + h + h1),
                text_color=(20, 45, 255),
            )
        ...
        if len(location_history) >= MAX_LOCATION_HISTORY_LENGTH:
            ...
            for id in list(discovered_ids):
                locations_with_id = [
                    location
                    for snapshot_locations in location_history
                ...
                if len(locations_with_id) > 0:
                    average_locations.append(
                        {
                            "id": id,
                            "image_x": util.average_value_for_key(
                                locations_with_id, "image_x", True, 1
                            ),
                            "image_y": util.average_value_for_key(
                                locations_with_id, "image_y", True, 1
                            ),
                            "x": util.average_value_for_key(
                                locations_with_id, "x", True, 1
                            ),
                            "y": util.average_value_for_key(
                                locations_with_id, "y", True, 1
                            ),
                            "orientation": util.average_value_for_key(
                                locations_with_id, "orientation", True, 1
                            )
                            % 360,
                        }
                    )
            ...
            [
                ...
                for location in average_locations
            ]
...
def get_closest_reference_point_offset(x, y, center_cell_offsets):
    ...
    try:
        return min(
            center_cell_offsets,
            key=lambda item: util.distance(
                x, y, item["reference_point_x"], item["reference_point_y"]
            ),
        )
    except:
        return None
...
def get_x_y_angle_offsets(x, y, center_cell_offsets):
    ...
    for i in range(4):
        closest_offsets.append(
            get_closest_reference_point_offset(x, y, remaining_offsets)
        )
    ...
    left_most_offset = min(
        closest_offsets[1:4], key=lambda offset: offset["reference_point_x"]
    )
    left_most_offset_delta = abs(
        closest_offset["reference_point_x"] - left_most_offset["reference_point_x"]
    )
    right_most_offset = max(
        closest_offsets[1:4], key=lambda offset: offset["reference_point_x"]
    )
    right_most_offset_delta = abs(
        right_most_offset["reference_point_x"] - closest_offset["reference_point_x"]
    )
    other_offset = util.compliment_of_list(
        closest_offsets, [left_most_offset, closest_offset, right_most_offset]
    )[0]
    if left_most_offset_delta > right_most_offset_delta:
        right_offset = closest_offset
        go_with_other = (
            abs(left_most_offset["reference_point_y"] - y)
            > abs(other_offset["reference_point_y"] - y)
            and other_offset["reference_point_x"] < closest_offset["reference_point_x"]
        )
        left_offset = other_offset if go_with_other else left_most_offset
    else:
        left_offset = closest_offset
        go_with_other = (
            abs(right_most_offset["reference_point_y"] - y)
            > abs(other_offset["reference_point_y"] - y)
            and other_offset["reference_point_x"] > closest_offset["reference_point_x"]
        )
        right_offset = other_offset if go_with_other else right_most_offset

    x_offset = linear_interpolation_with_2_ref_data_points(
        x, "reference_point_x", "x_offset", left_offset, right_offset
    )
    x_offset = 0 if x_offset == None else x_offset

    top_offset = max(
        center_cell_offsets, key=lambda offset: offset["reference_point_y"]
    )
    bottom_offset = min(
        center_cell_offsets, key=lambda offset: abs(offset["reference_point_y"] - y)
    )

    bottom_offset = None
    top_offset = None
    closest_offset = closest_offsets[0]
    bottom_most_offset = min(
        closest_offsets[1:4], key=lambda offset: offset["reference_point_y"]
    )
    bottom_most_offset_delta = abs(
        closest_offset["reference_point_y"] - bottom_most_offset["reference_point_y"]
    )
    top_most_offset = max(
        closest_offsets[1:4], key=lambda offset: offset["reference_point_y"]
    )
    top_most_offset_delta = abs(
        top_most_offset["reference_point_y"] - closest_offset["reference_point_y"]
    )
    other_offset = util.compliment_of_list(
        closest_offsets, [bottom_most_offset, closest_offset, top_most_offset]
    )[0]
    if bottom_most_offset_delta > top_most_offset_delta:
        top_offset = closest_offset
        go_with_other = (
            abs(bottom_most_offset["reference_point_x"] - x)
            > abs(other_offset["reference_point_x"] - x)
            and other_offset["reference_point_y"] < closest_offset["reference_point_y"]
        )
        bottom_offset = other_offset if go_with_other else bottom_most_offset
    else:
        bottom_offset = closest_offset
        go_with_other = (
            abs(top_most_offset["reference_point_x"] - x)
            > abs(other_offset["reference_point_x"] - x)
            and other_offset["reference_point_y"] > closest_offset["reference_point_y"]
        )
        top_offset = other_offset if go_with_other else top_most_offset

    y_offset = linear_interpolation_with_2_ref_data_points(
        y, "reference_point_y", "y_offset", bottom_offset, top_offset
    )
    y_offset = 0 if y_offset == None else y_offset

    angle_offset_x = linear_interpolation_with_2_ref_data_points(
        x, "reference_point_x", "angle_offset", left_offset, right_offset
    )
    angle_offset_x = 0 if angle_offset_x == None else angle_offset_x

    angle_offset_y = linear_interpolation_with_2_ref_data_points(
        y, "reference_point_y", "angle_offset", bottom_offset, top_offset
    )
    angle_offset_y = 0 if angle_offset_y == None else angle_offset_y

    x_offsets_distance = distance_from_2_ref_data_points(
        x, y, "reference_point_x", "reference_point_y", left_offset, right_offset
    )
    y_offsets_distance = distance_from_2_ref_data_points(
        x, y, "reference_point_x", "reference_point_y", bottom_offset, top_offset
    )

    angle_offset = 0
    if x_offsets_distance != None and y_offsets_distance != None:
        angle_offset = util.weighted_average(
            [angle_offset_x, angle_offset_y], [y_offsets_distance, x_offsets_distance]
        )
...
def linear_interpolation_with_2_ref_data_points(
    independent_variable_input,
    independent_variable_property,
    dependent_property,
    reference_point_1,
    reference_point_2,
):
    ...
    if point1_exists and point2_exists:
        result = linear_interpolation_with_2_ref_points(
            x1, y1, x2, y2, independent_variable_input
        )
    elif point1_exists:
        result = y1
    elif point2_exists:
...
def distance_from_2_ref_data_points(
    independent_variable_input,
    ...
    point2_exists = x2 != None and y2 != None
    if point1_exists and point2_exists:
        result = util.distance(
            independent_variable_input, dependent_variable_input, x1, y1
