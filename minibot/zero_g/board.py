...
class Board:
    ...
    def is_button_pressed(self) -> bool:
        ...
        :rtype: bool
        """
        return (not Pin(9, Pin.IN, Pin.PULL_UP).value()) or (not Pin(1, Pin.IN, Pin.PULL_UP).value())
        # res = ""
        # if not  == True:
        #     res += "True"
        # else:
    ...
    def wait_for_button(self):
        ...
        while self.is_button_pressed() != "False,False":
            time.sleep(.01)

        # Wait until user to release button before running
        while self.is_button_pressed() != "True,True":
            time.sleep(.01)
